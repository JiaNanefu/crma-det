...
def init_distributed():
    ...
    '''
    distributed_env = any(os.getenv(k) is not None for k in ('RANK', 'WORLD_SIZE', 'LOCAL_RANK'))
    ...
    try:
        ...
        if distributed_env:
            print('Failed to initialize distributed mode from environment variables.')
            print(f'{type(exc).__name__}: {exc}')
            raise

        print(f'Not init distributed mode. Reason: {type(exc).__name__}: {exc}')
        return False 
