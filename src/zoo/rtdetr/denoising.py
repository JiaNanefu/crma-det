...
def _validate_target_labels(targets, num_classes):
    labels = [t["labels"] for t in targets if len(t["labels"]) > 0]
    if not labels:
        return
    ...
    if min_label < 0 or max_label >= num_classes:
        raise ValueError(
            f"Target labels must be in [0, {num_classes - 1}] for num_classes={num_classes}; "
...
def get_contrastive_denoising_training_group(targets,
    ...
    if max_gt_num == 0:
        return None, None, None, None
    _validate_target_labels(targets, num_classes)
